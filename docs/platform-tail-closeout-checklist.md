...
- Clawith 平台优化线的 runbook 与 browser report 已补齐，可直接支撑 adoption/closeout。
...
| UI 若保留 inferred，只能作为草案/提示 | `src/pages/workspace/components/WorkspacePageContent.tsx` 对 inferred 资源展示 outline badge 与提示文案，不把它描述成正式判断来源 | 浏览器 `/workspace` 资源详情页提示文案 |
...
| 页面内可直接触发 doctor / backup / restore / rehearse | `src/shared/presentation/AuthorityOperatorControlPlaneCard.tsx` | `src/shared/presentation/AuthorityOperatorControlPlaneCard.test.tsx` |
| Connect/Settings 成为正式入口 | `src/pages/connect/Page.tsx`、`src/pages/settings/components/SettingsSections.tsx` | 浏览器 `/connect`、`/settings` smoke |
