...
import { classifyMetaSupportNeed } from "../../../../src/application/meta-agent/support-routing";
...
import { resolveMetaDepartment } from "../../../../src/domain/meta-agent/organization";
import type {
  Company,
...
export type CompanyOpsRunResult = {
  company: Company;
...
export function normalizeCompanyForOps(company: Company): Company {
  return {
    ...company,
...
export function applyHeartbeatStateUpdate(input: {
  company: Company;
  now: number;
...
export function runCompanyOpsCycle(input: {
  ...
  const seedCompany = normalizeCompanyForOps(input.company);
  ...
  const normalizedInputCompany = normalizeCompanyForOps({
    ...seedCompany,
  ...
  if (company.orgSettings?.autoCalibrate) {
    ...
    if (calibration.changed) {
      company = normalizeCompanyForOps({
        ...company,
