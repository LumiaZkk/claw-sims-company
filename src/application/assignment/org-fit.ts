...
export function buildOrgAdvisorSnapshot(company: Company): OrgAdvisorSnapshot {
  ...
  const hasMetaManagedBusinessLayer = nonMetaEmployees.some((employee) => {
    if (!employee.reportsTo || employee.reportsTo === ceoAgentId) {
      return false;
    }
    const manager = employeesById.get(employee.reportsTo);
    return Boolean(manager?.isMeta);
  });
  const operatingMode =
    departmentsWithLead === 0
      ...
      : hasMetaManagedBusinessLayer
        ? {
            key: "hybrid" as const,
            label: "混合分层",
            summary: "业务部门已经出现，但仍有一部分团队通过管理层节点中转，处于过渡态。",
          }
