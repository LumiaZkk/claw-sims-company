...
function isAgentReferenceKey(key: string) {
  return AGENT_REFERENCE_KEYS.has(key) || key.endsWith("AgentId") || key.endsWith("ActorId");
}

function isAgentReferenceArrayKey(key: string) {
  return AGENT_REFERENCE_ARRAY_KEYS.has(key) || key.endsWith("AgentIds") || key.endsWith("ActorIds");
}
...
export function rewriteKnownAgentReferences<T>(
  ...
  const visit = (node: unknown, key?: string): unknown => {
    if (Array.isArray(node)) {
      ...
      if (key && SESSION_KEY_ARRAY_KEYS.has(key)) {
        return node.map((entry) =>
    ...
    if (typeof node === "string") {
      ...
      if (key && SESSION_KEY_KEYS.has(key)) {
    ...
    return Object.fromEntries(
      Object.entries(node).map(([entryKey, entryValue]) => [
        entryKey,
