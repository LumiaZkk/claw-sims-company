...
function loadStoredProviderId(): string {
  const stored = storage.getItem(BACKEND_PROVIDER_KEY);
...
function loadLegacyGatewayConfig():
  ...
  | null {
  const raw = storage.getItem(LEGACY_GATEWAY_CONFIG_KEY);
  if (!raw) {
    return null;
  }
...
function migrateLegacyGatewayConfig(providerId: string) {
  ...
  storage.setItem(BACKEND_PROVIDER_KEY, providerId);
  if (!storage.getItem(providerUrlKey(providerId))) {
    storage.setItem(providerUrlKey(providerId), legacyConfig.url);
  }
  if (!storage.getItem(providerTokenKey(providerId))) {
    storage.setItem(providerTokenKey(providerId), legacyConfig.token);
  }
  if (legacyConfig.url) {
    storage.setItem(GATEWAY_CONNECTED_ONCE_KEY, '1');
  }
}
...
export const useGatewayStore = create<GatewayState>((set) => {
  ...
  return {
    ...
    markModelsRefreshed: (version?: number) => {
      ...
      storage.setItem(GATEWAY_MODELS_VERSION_KEY, String(nextVersion));
