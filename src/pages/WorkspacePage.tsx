...
import type { ArtifactRecord, SharedKnowledgeItem, WorkItemRecord } from "../features/company/types";
import {
...
function getKnowledgeSourceFiles(
...
export function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    const selectedApp = workspaceApps.find((app) => app.id === selectedAppId) ?? workspaceApps[0] ?? null;
    ...
    const preferredFiles =
      selectedApp?.kind === "knowledge-hub"
        ? getKnowledgeSourceFiles(selectedKnowledgeItem, workspaceFiles)
        : workspaceFiles;
    setSelectedFileKey(
      pickDefaultFile(
        preferredFiles,
  ...
  }, [knowledgeItems, selectedAppId, selectedFileKey, selectedKnowledgeId, workspaceApps, workspaceFiles]);
  ...
  const selectedKnowledgeItem =
    ...
    knowledgeItems[0] ??
    null;
  const selectedKnowledgeSourceFiles = getKnowledgeSourceFiles(selectedKnowledgeItem, workspaceFiles);
  const selectedFile =
  ...
  const knowledgeFiles = workspaceFiles.filter((file) => file.kind === "knowledge");
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                  ...
                  {selectedKnowledgeItem.sourceAgentId ? (
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => navigate(`/chat/${encodeURIComponent(selectedKnowledgeItem.sourceAgentId!)}`)}
                    >
      ...
      <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-4">
                ...
                <div className="mt-2 text-2xl font-semibold text-slate-950">
                  {chapterFiles.length + canonFiles.length + knowledgeFiles.length}
                </div>
                <div className="mt-1 text-sm text-slate-600">
                  {chapterFiles.length + canonFiles.length + knowledgeFiles.length > 0
                    ? "当前公司已经有可直接阅读的正文、设定或知识产物。"
        ...
        <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <div className="space-y-5">
            ...
            {selectedApp.kind === "knowledge-hub" ? renderKnowledgeHub() : null}
