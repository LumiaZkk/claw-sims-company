...
import {
} from "./chat-session-runtime-helpers";
...
export function useChatSessionRuntime(input: ChatSessionRuntimeInput) {
  ...
  const streamTextRef = input.streamTextRef ?? { current: null };
  const activeRunIdRef = input.activeRunIdRef ?? { current: null };
  const pendingGenerationStartedAtRef = input.pendingGenerationStartedAtRef ?? { current: null };
    ...
    const unsubscribe = gateway.subscribe("chat", (rawPayload) => {
      ...
      if (payload.state === "delta") {
        ...
        if (deltaText && deltaText.length >= (streamTextRef.current?.length ?? 0)) {
          ...
          if (!pendingGenerationStartedAtRef.current) {
  ...
  useEffect(() => {
    ...
    const hydrateTerminalHistory = (runId: string | null | undefined) => {
      ...
      const hydrationKey = [
        ...
        pendingGenerationStartedAtRef.current ?? 0,
      ].join("|");
      ...
      void (async () => {
        try {
          for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            const pendingSince = pendingGenerationStartedAtRef.current ?? 0;
    ...
    const unsubscribe = gateway.subscribe("*", (raw) => {
      ...
      if (
        ...
        runtimeEvent.runId &&
        runtimeEvent.runId !== activeRunIdRef.current
      ) {
        return;
      }
      ...
      if (streamUpdate) {
        ...
        const previousText = streamTextRef.current ?? liveSession?.streamText ?? "";
        const nextText = resolveMergedRuntimeAssistantText({
          previousText,
          nextText: streamUpdate.text,
          nextDelta: streamUpdate.delta,
        });
        const startedAt = pendingGenerationStartedAtRef.current ?? liveSession?.startedAt ?? Date.now();
        const runId = runtimeEvent.runId ?? activeRunIdRef.current ?? liveSession?.runId ?? null;
        if (!pendingGenerationStartedAtRef.current) {
  ...
  useEffect(() => {
    ...
    const syncHistoryPreview = () => {
      ...
      const liveSession = readLiveChatSession(input.activeCompany?.id, currentSessionKey);
      const pendingSince = pendingGenerationStartedAtRef.current ?? liveSession?.startedAt ?? 0;
      const isGenerating =
        Boolean(activeRunIdRef.current)
        || Boolean(pendingGenerationStartedAtRef.current)
