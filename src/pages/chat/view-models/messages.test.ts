...
describe("buildChatDisplayItems", () => {
    ...
    expect(buildChatDisplayItems(messages)).toEqual([
      {
        ...
        detailContent: "Successfully wrote 285 bytes to /tmp/TASK-BOARD.md",
        tone: "sky",
        count: 1,
      },
    ]);
  ...
  it("keeps regular assistant content visible while collapsing tool chatter", () => {
    ...
    expect(items[1]).toMatchObject({
      ...
      detail: "已返回结构化结果",
      detailContent: "{\"ok\":true,\"status\":\"sent\"}",
      tone: "sky",
    });
  });

  it("keeps full preview tool results available for disclosure", () => {
    const items = buildChatDisplayItems([
      {
        role: "toolResult",
        toolName: "authority.company.employee.preview_hire",
        text: [
          "Preview ready: 3 candidate templates, blank fallback kept",
          "",
          "## Candidate Templates",
          "1. Designer (tm-template-designer) | score=0.90 | confidence=0.90 | auto=yes",
          "   reasons: 角色描述与模板标题匹配",
          "   gaps: 无",
        ].join("\n"),
        timestamp: 1_200,
    ...
    expect(items).toEqual([
      expect.objectContaining({
        kind: "tool",
        title: "authority.company.employee.preview hire 已返回结果",
        detail: "Preview ready: 3 candidate templates, blank fallback kept",
        detailContent: expect.stringContaining("## Candidate Templates"),
      }),
    ]);
  });

  it("does not collapse distinct tool results when their full details differ", () => {
    const items = buildChatDisplayItems([
      {
        role: "toolResult",
        toolName: "authority.company.employee.preview_hire",
        text: "Preview ready: 1 candidate template, blank fallback kept\n\n## Candidate Templates\n1. Designer",
        timestamp: 1_300,
      },
      {
        role: "toolResult",
    ...
    expect(items[0]).toMatchObject({
      kind: "tool",
      detailContent: expect.stringContaining("Designer"),
      count: 1,
    });
    expect(items[1]).toMatchObject({
      kind: "tool",
      detailContent: expect.stringContaining("Writer"),
      count: 1,
    });
  });
