...
import type { AgentTemplateDefinition, TemplateMatch } from "../domain/org/types";
import { buildHireIntentFromManualInput, matchTalentTemplates } from "../domain/org/talent-market";

export type HireTemplateSelection = {
...
export type HireConfig = {
  ...
  traits: string;
  avatarFile?: File;
  templateSelection?: HireTemplateSelection;
};
...
export function ImmersiveHireDialog({
  open,
  onOpenChange,
  onSubmit,
  busy,
  ...
  const [selectedTemplateMatch, setSelectedTemplateMatch] = useState<TemplateMatch | null>(null);
  ...
  const matchCandidates = useMemo(() => {
    if (!companyId || templates.length === 0 || !role.trim() || !description.trim()) {
      return [];
    }
    ...
    return matchTalentTemplates(intent, templates).slice(0, 3);
  }, [budget, companyId, description, modelTier, role, templates, traits]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (step === STEPS.length - 1 && isStep1Valid) {
      const templateSelection: HireTemplateSelection = selectedTemplateId
        ? { templateId: selectedTemplateId, sourceType: "template", match: selectedTemplateMatch }
        : { templateId: null, sourceType: "blank" };
      void onSubmit({
        role,
        description,
        modelTier,
                        ...
                        {templates.length > 0 ? (
                          <div className="pt-4 space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-zinc-300">推荐模板</span>
                              {selectedTemplateId ? (
                                <button
                                  type="button"
                                ...
                                {matchCandidates.map((match) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="mt-1 text-[10px] text-zinc-500">{template.summary}</div>
                                      ...
                                      <div className="mt-2">
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            setSelectedTemplateMatch(match);
                                            if (!role.trim()) {
                                              setRole(template.title);
                                            }
                                            if (!description.trim()) {
                                              setDescription(template.summary);
                                            }
                                            if (template.defaultTraits) {
                                              setTraits(template.defaultTraits);
                                            }
                                            if (template.recommendedModelTier) {
                                              setModelTier(template.recommendedModelTier);
                                            }
                                            if (typeof template.defaultBudgetUsd === "number") {
                                              setBudget(template.defaultBudgetUsd);
                                            }
                                          }}
                                        >
                                          {isSelected ? "已应用模板" : "应用模板"}
                                        </button>
                                      </div>
                                    </div>
                                  );
